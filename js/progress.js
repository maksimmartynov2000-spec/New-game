...
const Progress = (() => {
    ...
    function emptyState() {
        return {
            ...
            // Прогресс текущего пазла. БОЛЬШЕ НЕ ИСПОЛЬЗУЕТСЯ: картинка закреплена за
            // клеткой, а кусочки выводятся из числа верных ответов в ней (см. index.html,
            // puzzleFilledFor). Поле оставлено, чтобы не выбрасывать данные из уже
            // сохранённых состояний, — читать его никто не должен.
    ...
    //                                        серия подряд сейчас, лучшая серия за день] }
    function emptyDay() {
        ...
        // tr — верные ответы, данные В РЕЖИМЕ ОБУЧЕНИЯ, с подсказкой на экране.
        //      Лежат ОТДЕЛЬНО от c намеренно: в точность, скорость, лесенки, пазл и
        //      разбор по классам они не идут — иначе цифры начали бы означать «умею
        //      с подсказкой», а не «умею». Зато дневная цель их засчитывает: ребёнок
        //      занимался, и режим, который рвёт серию дней, никто не включит.
        //      Старые записи поля не имеют — Object.assign(emptyDay(), raw) в
        //      санитайзере добивает его нулём.
        return { c: 0, w: 0, a: 0, s: 0, p: 0, ms: 0, mc: 0, tr: 0, t: {}, e: {}, te: {} };
    }
    ...
    // Когда в последний раз удалось слить состояние с серверным. Нужно прощальной
    // записи (см. farewell): она пишет БЕЗ слияния, и знать, насколько давно мы
    // видели серверную копию, — единственный способ ограничить её последствия.
    let lastMergeAt = 0;
    // Насколько свежим должно быть слияние, чтобы прощальная запись считалась
    // безопасной. Минута выбрана с запасом: во время игры слияние идёт каждые
    ...
    function mergeState(a, b) {
        ...
        days.forEach(k => {
            ...
            tk.forEach(key => {
                const p = (x.t && x.t[key]) || [];
                const q = (y.t && y.t[key]) || [];
                // [верно, ошибок, почти, суммарное время верных (мс), сколько ответов
                //  в этом времени, серия подряд сейчас, лучшая серия за день]
                //
                // Серии сливаются тем же максимумом, что и всё остальное в дне. Для
                // лучшей это ровно верно. Для идущей — приблизительно: два устройства
                // в один день считают её порознь. Заводить ради этого особое правило
                // незачем: занимаются с одного устройства, а ошибка в большую сторону
                // здесь безобиднее, чем потерянное задание.
                const slot = [
                    Math.max(p[0] || 0, q[0] || 0),
                    Math.max(p[1] || 0, q[1] || 0),
                    Math.max(p[2] || 0, q[2] || 0),
                    ...
                    Math.max(p[5] || 0, q[5] || 0),
                    Math.max(p[6] || 0, q[6] || 0)
                ];
                bucket.t[key] = slot;
            });
            out.daily[k] = bucket;
    ...
    function normalize(raw) {
        ...
        s.puzzle.filled = num(s.puzzle.filled);
    ...
    return {
        ...
        // --- текущий пазл ---
        // getPuzzle/setPuzzle убраны: прогресс пазла больше не хранится — картинка
        // закреплена за клеткой, а кусочки выводятся из числа верных ответов в ней.
        // Само поле state.puzzle осталось, чтобы не выбрасывать уже сохранённые данные.
        ...
        recordAnswer(topicKey, outcome, elapsedMs) {
            ...
            if (isRight) day.mc++;
            if (topicKey) {
                const slot = day.t[topicKey] || (day.t[topicKey] = [0, 0, 0, 0, 0, 0, 0]);
                while (slot.length < 7) slot.push(0); // запись могла прийти из старой версии
                if (isRight) slot[0]++; else slot[1]++;
                if (kind === 'almost') slot[2]++;
                slot[3] += spent;
                if (isRight) slot[4]++;
                // Серия верных подряд в этой клетке за день: [5] — идущая сейчас,
                // [6] — лучшая за день. Из сумм её не восстановить: в них нет порядка,
                // а «десять подряд» — это именно про порядок.
                //
                // Хранятся обе, и это не запас: по идущей рисуется полоска (после
                // ошибки она честно падает в ноль на глазах), а по лучшей решается,
                // выполнено ли задание, — иначе выполненное задание разавтоматилось
                // бы от следующей же ошибки.
                //
                // «Почти» серию не рвёт: счёт верный, не сокращена запись, и во всех
                // остальных местах приложения этот ответ считается верным.
                if (isRight) { slot[5]++; if (slot[5] > slot[6]) slot[6] = slot[5]; }
                else slot[5] = 0;
            }

            persistLocal();
        },
        // Верный ответ в режиме обучения. Пишет РОВНО ОДИН счётчик и больше ничего:
        // ни totals, ни byTopic, ни времени, ни серии по клетке. Сделано отдельным
        // методом, а не флагом внутри recordAnswer, именно поэтому — перепутать
        // аргумент и протащить обучение в статистику тут нечем.
        ...
        // нашей, записать обратно. Когда страницу убивают, до конца доходит хорошо
        // если первый: на телефоне вкладку закрывают не кнопкой, и времени у нас
        // нет. Здесь шаг один, и запрос помечается keepalive — такой браузер обязан
        // дослать даже после смерти страницы.
        //
        // Цена одного шага — запись БЕЗ слияния, то есть серверная копия
        // перезаписывается нашей. Отсюда две страховки:
        //   1) отправляем, только если слияние было не дольше минуты назад. Тогда
        //      перезаписать можно разве что то, что другое устройство записало за
        //      эти секунды, — а во время игры слияние идёт каждые восемь секунд;
        //   2) размер и наличие токена проверяет сам драйвер: keepalive молча
        //      роняет тела больше 64 КиБ, а обменивать пароль на токен уже некогда.
        //
        // Не сработало — не беда: локальная копия уже на диске, и она доедет до
        // сервера при следующем запуске. Возвращаем false, чтобы вызывающий код
        // попробовал обычный flush().
        //
        // dirty здесь НЕ сбрасывается намеренно: запрос отправлен, но дошёл ли он —
        // мы уже не узнаем, а лишняя запись при следующем запуске безвредна.
        farewell() {
