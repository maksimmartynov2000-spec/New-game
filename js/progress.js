...
const Progress = (() => {
    ...
    // Сколько кусочков в картинке. То же число, что PUZZLE_TOTAL в index.html, но
    // взять его отсюда нельзя: этот файл подключается раньше и обязан работать сам
    // по себе. Чтобы числа не разъехались, их равенство сторожит проверка в
    // test/puzzles.test.js — менять придётся оба места разом.
    ...
    function emptyState() {
        return {
            ...
            // Снят ли разовый сдвиг кусочков, оставшийся от правила «−2 за ошибку».
            // Подробности — в normalize(), там же он и снимается.
            puzzleAligned: false,
    ...
    function mergeState(a, b) {
        ...
        // Выравнивание кусочков делается один раз, и флаг об этом обязан пережить
        // слияние: out собирается из emptyState(), где он false, и без этой строки
        // состояние, пришедшее с сервера, каждый раз выглядело бы невыровненным.
        out.puzzleAligned = !!(a.puzzleAligned || b.puzzleAligned);
    ...
    function normalize(raw) {
        ...
        // Разовое выравнивание кусочков пазла.
        //
        // До #160 за ошибку отнимали два кусочка, поэтому у всех, кто играл раньше,
        // кусочков выдано МЕНЬШЕ, чем было верных ответов: у наших учеников от двух
        // до ста с лишним. Сдвиг постоянный — сам он никуда не денется, и картинка
        // у такого ученика будет заканчиваться не на сотом ответе, а позже, всегда.
        //
        // Флаг нужен именно потому, что правило «кусочков столько же, сколько верных
        // ответов» не вечное: ответ «почти» (верная, но несокращённая дробь) считается
        // верным и кусочка не даёт. Без флага каждая такая дробь молча добавляла бы
        // кусочек здесь — это была бы уже другая игра, а не выравнивание.
        //
        // Счётчик только растёт: если кусочков почему-то выдано больше, чем должно,
        // оставляем как есть. Прогресс назад не ходит нигде, и здесь тоже не пойдёт.
        if (!s.puzzleAligned) {
            s.puzzleAligned = true;
            const owed = s.totals.correct
                - (s.totals.puzzlesCompleted * PUZZLE_PIECES + s.puzzle.filled);
            if (owed > 0) {
                // Больше одной картинки разом не выдаём: собранная картинка — это ещё
                // и карточка в коллекции, а её ученик не зарабатывал. Остаток срезается.
                s.puzzle.filled = Math.min(PUZZLE_PIECES - 1, s.puzzle.filled + owed);
            }
        }
