...
const MISTAKE_LABELS = {
    'не сократил':                 t('Ответ не сокращён'),
    'общий знаменатель зря':       t('Общий знаменатель тут не нужен'),
    'сложил знаменатели':          t('Знаменатели сложены (вычтены) тоже'),
    'перепутал действие':          t('Не то действие'),
    'не перевернул дробь':         t('При делении вторая дробь не перевёрнута'),
    'забыл целую часть':           t('Целая часть потеряна'),
    'ошибся в переводе':           t('Ошибка при переводе дроби'),
    ...
    'оба разряда мимо':            t('Не сошлись ни десятки, ни единицы'),
    'промахнулся рядом':           t('Ответ рядом, но не тот'),
    'не умножил десятки':          t('Умножено только на единицы'),
    'нет решения зря':             t('Решение есть, а выбрано «нет решения»'),
    'ноль в примере':              t('Ноль в примере не учтён'),
    'не перенёс десяток':          t('Десяток не перенесён'),
    'не занял десяток':            t('Десяток при вычитании не занят'),
    'переставил числа':            t('Числа поменялись местами'),
    'взял одно из чисел':          t('Это одно из чисел примера'),
    'сложил вместо умножения':     t('Сложение вместо умножения'),
    'не выровнял запятую':         t('Запятые перед сложением не выровнены'),
    'забыл поделить':              t('Умножено, но не поделено на знаменатель'),
    'нашёл только одну долю':      t('Найдена одна доля, но не умножена'),
    'нашёл остаток':               t('Это остаток, а не сама доля'),
    'ошибся на одну долю':         t('Мимо на одну долю'),
    'забыл перемножить знаменатели': t('Числители перемножены, знаменатели нет'),
    'перевернул не ту дробь':      t('Перевёрнута первая дробь вместо второй'),
    'перевёл не обе дроби':        t('К общему знаменателю приведена только одна дробь'),
...
const MISTAKE_SHORT = {
    'не сократил':                 t('не сокращено'),
    'общий знаменатель зря':       t('лишнее приведение'),
    'сложил знаменатели':          t('знаменатели тоже'),
    'перепутал действие':          t('другое действие'),
    'не перевернул дробь':         t('без переворота'),
    ...
    'оба разряда мимо':            t('оба разряда'),
    'промахнулся рядом':           t('мимо рядом'),
    'не умножил десятки':          t('без десятков'),
    'нет решения зря':             t('зря «нет решения»'),
    ...
    'сложил вместо умножения':     t('сложение вместо ×'),
    'не выровнял запятую':         t('запятые не выровнены'),
    'забыл поделить':              t('без деления'),
    ...
    'перевёл не обе дроби':        t('приведена одна'),
...
function classifyIntegerLike(problem, correct, chosen, opKey, isNegative) {
    ...
    // Двузначное на однозначное умножают по частям: сначала на единицы, потом
    // на десятки, потом складывают. Пропустить вторую часть — ровно промах на
    // 10·(меньший множитель): 7 × 13 = 91, а посчитано 7 × 3 = 21. Это не
    // соседняя клетка таблицы и не случайный промах, а один конкретный
    // пропущенный шаг, и ловушка с таким числом ставится намеренно —
    // см. sameUnitsDecoy в генераторе.
    //
    // Только ВНИЗ. Тот же промах вверх означал бы лишний десяток, а не
    // потерянный, и объяснять его этими словами было бы неправдой; вверх
    ...
    if (!isNegative && haveOperands && opKey === 'mul') {
        ...
        if (hi >= 11 && hi % 10 !== 0 && lo > 1 && lo <= 9
            ...
            return 'не умножил десятки';
        }
    }

    // Та же последняя цифра, другой десяток. У сложения это «ошибка в десятках»
    // и разбирается выше по точному промаху на десять; у умножения и деления
    // промах кратен десяти, но не равен ему (7 × 13 против 7 × 3 — это 70),
    ...
    // Не сошлась ни одна цифра. Это третий угол квадрата ловушек, который
    // строит генератор для сложения и вычитания (см. addSubDecoyPlan): два
    // других его угла разбираются правилами выше, а этот до сих пор уходил
    // в «другую ошибку» — и составлял её большую часть.
    //
    ...
    // Ответ рядом, но не соседний. На однозначных ответах генератор ставит
...
function classifyMistake(meta, problem, correct, chosen) {
    ...
    try {
        ...
        // Обратная путаница: решение есть, а выбрано «нет решения». Вариант
        // этот стоит на экране намеренно (см. buildDistractors) и означает ту же
        // нетвёрдость в правиле, только с другой стороны: запрещено делить НА
        // ноль, а не делить ноль и не делить вообще.
        //
        // Отдельным видом, а не общим с «делил на ноль»: репетитору важно, в
        // какую сторону ученик путает — не применил правило или применил не к
        // месту. До сих пор этот выбор уходил в «другую ошибку», потому что
        // разбор целых чисел берёт только числа, а здесь ответ строковый.
        //
        // СТРОГО ПОСЛЕ разбора нуля. У примера «0 ÷ 10» выбор «нет решения» —
        // это уже названная ошибка «ноль в примере»: путаница между делением
        // ноля и делением на ноль. Стой эта проверка выше, она перехватывала бы
        // её и подменяла более точное имя менее точным.
        if (chosen === 'NO_SOLUTION' && !meta.isNegative) return 'нет решения зря';

        return classifyIntegerLike(problem, correct, chosen, opKey, !!meta.isNegative)
               || 'другая ошибка';
    } catch (e) {
...
function hintArgs(kind, meta, problem, correct, chosen) {
    ...
    if (kind === 'оба разряда мимо') {
        if (typeof correct !== 'number' || typeof chosen !== 'number') return null;
        return { game: [], review: [chosen, correct, u(correct), Math.floor(correct / 10)] };
    }
    if (kind === 'не умножил десятки') {
        const lo = Math.min(a, b), hi = Math.max(a, b);
        // Те же условия, что и у самого вида: без них подстановки соврут.
        if (hi < 11 || hi % 10 === 0 || lo <= 1 || typeof correct !== 'number') return null;
        const tens = Math.floor(hi / 10) * 10;
        return { game: [lo, u(hi)],
                 review: [lo, u(hi), lo * u(hi), tens, lo * tens, correct] };
    }
    if (kind === 'нет решения зря') {
        if (b === 0 || typeof correct !== 'number') return null;
        return { game: [b], review: [a, b, correct] };
    }
    return null;
}
...
function trickPick(meta, problem) {
    ...
    // ===== ОТРИЦАТЕЛЬНЫЕ =====
    //
    // До этой правки режим обучения на отрицательных МОЛЧАЛ: замер по генератору дал
    // 0% подсказок у сложения и вычитания на всех пяти звёздах и 3–5% у умножения.
    // Причина простая: приёмы ниже разбирают класс примера ('1', '2', 'h'), а у
    // отрицательных классы совсем другие — 'same', 'near', 'cross' — и ни одна ветка
    // не срабатывала. Деление сюда не входит: общий приём «на что умножить» работает
    // с отрицательными как есть и учит именно знаку (90–99% покрытия).
    //
    // Приёмы здесь — школьные правила знаков, и каждый останавливается за шаг до
    // ответа: последний счёт ученик делает сам.
    if (meta.isNegative) {
        if (op === 'add' || op === 'sub') {
            // Приводим к виду «x + y» — ровно так же, как это делает structuralClassOf:
            // второе слагаемое достаём из ответа, чтобы разбор не зависел от того,
            // перевернул ли генератор знак при показе.
            const ans = problem.answer;
            if (typeof ans !== 'number') return null;
            const x = a, y = ans - a;
            if (x === 0 || y === 0) return null;   // ноль — не правило знаков
            if (Math.sign(x) === Math.sign(y)) {
                // Знак не называем словом: он и так виден в примере, а назвать его
                // значит подсказать половину ответа там, где вся задача — про знак.
                return { key: 'neg:same', args: [Math.abs(x), Math.abs(y)] };
            }
            const бол = Math.abs(x) >= Math.abs(y) ? x : y;
            const мен = Math.abs(x) >= Math.abs(y) ? y : x;
            // Модули равны — числа гасят друг друга в ноль. Вычитать нечего, и
            // «из 7 вычти 7» было бы не приёмом, а самим ответом.
            if (Math.abs(бол) === Math.abs(мен)) return null;
        ...
        // Деление падает ниже, в общий приём.
    }
    ...
    if (op === 'sub') {
        ...
        if (struct.cls === '1' || struct.cls === '2') {
            ...
            // Шаг «займи десяток» ИНОГДА СОВПАДАЕТ СО ВСЕМ ПРИМЕРОМ — и тогда подсказка
            // печатает ученику готовый ответ. У 23 − 7 шаг это 13 − 7 = 6, а ответ 16:
            // всё честно. У 12 − 4 шаг это 12 − 4 = 8, то есть сам пример и сам ответ.
            //
            // Сравниваем прямо: шаг и ответ. Можно было написать «десятки первого на
            // один больше десятков второго» — это равносильно, проверено перебором всех
            // 8730 пар с заёмом до 200. Но прямое сравнение говорит ровно то, что нас
            // волнует, работает с любыми знаками и не разъедется, если однажды
            // поменяется сам приём.
            //
            // Замер до этой правки, по настоящему генератору: на 1★ ответ печатался в
            // 100% подсказок вычитания, на 2★ в 94,6%. Нашёл это не тест, а Максим на
            // уроке с учеником.
            if (top - low === a - b) {
                const tail = a % 10;      // хвост до круглого
                const rest = b - tail;    // сколько останется отнять после спуска
                // Спуск до круглого — приём получше переворота, но годится не всегда:
                //   tail > 0        у 10 − 6 спускаться некуда, число уже круглое;
                //   rest < 10       у 64 − 57 остаток вышел бы 53, а «отнять 53 от 60»
                //                   тяжелее самого примера;
                //   rest !== a - b  у 14 − 9 остаток 5 и ответ 5 — спуск сам начал бы течь.
                if (tail > 0 && rest > 0 && rest < 10 && rest !== a - b) {
                    return { key: 'sub:ten', args: [tail, a - tail, rest] };
                }
                // Переворот в сложение. Печатаются только два числа самого примера,
                // поэтому новой информации не выдаётся в принципе: утечка невозможна
                // по устройству, а не по удаче. Ровно так же устроена подсказка деления.
                return { key: 'sub:toAdd', args: [b, a] };
            }
    ...
    if (op === 'div') {
        ...
        // И только если сокращённый делитель НЕ СОВПАЛ С ОТВЕТОМ: у 40 ÷ 20 подсказка
        // «Убери по нулю: 4 ÷ 2» печатает двойку, а два и есть ответ, и ребёнку,
        // который просто читает последнее число, считать уже нечего. Та же болезнь,
        // что была у спуска к десятку и у вычитания через сотню. Редко — около 2% всех
        // подсказок деления — и чинится тем же: уходим в общий приём, где печатаются
        // только числа самого примера. Молчания при этом не появляется.
        // Сравниваем МОДУЛИ: у −160 ÷ 40 сокращённый делитель 4, а ответ −4, и знаки
        // разные — но на экране всё равно стоит четвёрка рядом с ответом «минус четыре».
        if (a % 10 === 0 && b % 10 === 0 && Math.abs(b / 10) !== Math.abs(a / b)) {
            return { key: 'div:round', args: [a / 10, b / 10] };
        }
