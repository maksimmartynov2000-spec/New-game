...
group('Десятичные: в примере всегда есть запятая');
// В разделе десятичных дробей пример вроде «6 × 3» — чужой: решить его можно, а
// потренировать десятичные — нет. Раньше такие пропускались намеренно: в подборе
// величин стояло послабление «оба ноль знаков — редкость (<5%)» на 1★ и «<2%» на 2★,
// и это давало 1.7% и 0.4% примеров без единой запятой. Плюс в делении такой пример
// мог возникнуть уже ПОСЛЕ сокращения («2×5=10» при dp=1 даёт просто «1»).
// Послабления убраны, проверка держит результат.
test('ни один пример не состоит из двух целых чисел', () => {
    const bad = [];
    ['add', 'sub', 'mul', 'div'].forEach(op => {
        for (let lvl = 1; lvl <= 5; lvl++) {
            for (let i = 0; i < 3000; i++) {
                const p = G.generateDecimalProblem(op, lvl);
                if (p.d1.dp === 0 && p.d2.dp === 0) {
                    bad.push(`${op} ${lvl}★: ${G.decToString(p.d1, false)} ${op} ${G.decToString(p.d2, false)}`);
                }
            }
        }
    });
    assert(bad.length === 0, `пример без запятой: ${bad.slice(0, 3).join('; ')} (всего ${bad.length} из 60000)`);
});

// Ноль в примере — отдельный, намеренно оставленный случай: «0 × 0.7» и «0 ÷ 0.7»
// выпадают примерно в полпроцента и учат тому, что ноль остаётся нулём. Здесь важно,
// что ВТОРОЕ число при этом всё равно десятичное — иначе получилось бы «0 × 7».
test('в примерах с нулём второе число всё равно с запятой', () => {
    const bad = [];
    ['mul', 'div'].forEach(op => {
        for (let lvl = 1; lvl <= 5; lvl++) {
            for (let i = 0; i < 3000; i++) {
                const p = G.generateDecimalProblem(op, lvl);
                const hasZero = p.d1.intVal === 0 || p.d2.intVal === 0;
                if (hasZero && p.d1.dp === 0 && p.d2.dp === 0) {
                    bad.push(`${op} ${lvl}★: ${G.decToString(p.d1, false)} ${op} ${G.decToString(p.d2, false)}`);
                }
            }
        }
    });
    assert(bad.length === 0, `ноль без запятой рядом: ${bad.slice(0, 3).join('; ')}`);
});
