...
// ============ Режим никогда не включается сам ============
//
// Ответы с подсказкой не идут ни в точность, ни в скорость, ни в лесенки, ни в пазл.
// Раньше режим предлагался автоматически на новой клетке — и человек, не заметивший
// включённого переключателя, решал сотню примеров впустую. Цена ошибки несимметрична:
// не включить подсказки там, где они были нужны, — это миссия потруднее; включить их
// молча — это стёртый час работы.

test('режим обучения не включается сам ни при каких условиях', () => {
    // Никакой автоподстановки в коде остаться не должно: ни функции, ни её вызова,
    // ни порога «клетка новая». Проверка по исходнику, а не по поведению, потому что
    // вернуть это можно одной строкой в любом месте экрана выбора.
    const следы = ['syncTrainSuggestion', 'trainSuggestedFor', 'TRAIN_SUGGEST_BELOW'];
    const найдено = следы.filter(x => SCRIPT.indexOf(x) >= 0);
    eq(найдено.join(', '), '',
        'автоподстановка режима вернулась: ' + найдено.join(', '));
});

test('объявление режима — выключено', () => {
    const m = SCRIPT.match(/let trainWanted = (\w+);/);
    assert(m, 'не найдено объявление trainWanted');
    eq(m[1], 'false', 'режим объявлен включённым');
});

test('сброс выбора миссии гасит подсказки', () => {
    // Включённый однажды переключатель не должен переживать смену клетки: иначе он
    // тихо съест статистику следующей миссии.
    const m = SCRIPT.match(/function resetConfigScreen\(\)[\s\S]*?\n        \}/);
    assert(m, 'resetConfigScreen не найдена — срез сломался');
    assert(/trainWanted = false/.test(m[0]),
        'сброс миссии не гасит режим обучения');
});
