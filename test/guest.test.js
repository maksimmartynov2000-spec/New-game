...
test('прогресс переезжает целиком', async () => {
    ...
    // Именно 'solo', а не 'self': 'self' означает репетитора, и вчерашний гость
    // получил бы доступ ко всем разделам, включая те, что ещё в разработке.
    eq(P.get().accountType, 'solo', 'аккаунт из гостя заведён с правами репетитора');
});
...
group('То, что сломалось у живого человека');

test('полный доступ есть только у репетитора', () => {
    // Раньше правило звучало «не ученик — значит всё открыто», и это было верно, пока
    // «self» означало только репетитора. С гостем и самостоятельной регистрацией под то
    // же слово попали дети с улицы — и получили права репетитора: им открывались
    // отрицательные, десятичные и дроби, то есть разделы, которые ещё в разработке.
    const m = HTML.match(/function hasFullAccess\(\)[\s\S]*?\n        \}/);
    assert(m, 'hasFullAccess не найдена — срез сломался');
    assert(/type === 'solo'\) return false/.test(m[0]),
        'самостоятельный игрок снова получает полный доступ ко всем разделам');
    assert(/type === 'self'\) return true/.test(m[0]),
        'репетитор потерял полный доступ — это сломает ему работу с учениками');
});

test('ворота по звёздам действуют и на самостоятельных', () => {
    // Иначе зарегистрировавшийся сам открывал бы все пять звёзд сразу, а ученик
    // репетитора — нет. Правила должны быть одни.
    const m = HTML.match(/function levelGateApplies\(\)[\s\S]*?\n        \}/);
    assert(m, 'levelGateApplies не найдена — срез сломался');
    assert(/'solo'/.test(m[0]),
        'самостоятельному игроку звёзды открыты без счёта, в отличие от ученика');
});

test('в базе самостоятельный аккаунт заводится как solo', () => {
    // Если сервер поставит 'self', приложение сочтёт ребёнка репетитором при первом же
    // входе с другого устройства — там локальной копии нет, и тип приедет с сервера.
    assert(/'solo'/.test(SQL),
        'session_register всё ещё заводит аккаунт с типом репетитора');
    assert(!/\n    'self',/.test(SQL),
...
test('регистрация открывается поверх профиля, а не под ним', () => {
    ...
    const reg = HTML.match(/id="registerScreen"[^>]*z-index:\s*(\d+)/);
    const prof = HTML.match(/id="profileScreen"[^>]*z-index:\s*(\d+)/);
    assert(reg && prof, 'не найдены слои экранов регистрации и профиля');
    assert(Number(reg[1]) > Number(prof[1]),
        `регистрация (${reg[1]}) ниже профиля (${prof[1]}) — она откроется под ним`);
});

test('закрытая регистрация обязательно куда-то приводит', () => {
    // Куда именно — проверяет test/auth-doors.test.js в настоящем браузере. Здесь только
    // то, что решение принимается по тому, ОТКУДА пришли, а не по одному признаку
    // «из профиля»: на этом одном признаке гость оставался без единого экрана.
    const m = HTML.match(/function returnFromRegister\(\)[\s\S]*?\n        \}/);
    assert(m, 'returnFromRegister не найдена — срез сломался');
    assert(/registerReturnTo === 'profile'/.test(m[0]) && /registerReturnTo === 'game'/.test(m[0]),
        'возврат не различает, откуда позвали регистрацию');
    assert(/configScreen/.test(m[0]),
        'нет ветки «обратно в игру» — закрытая регистрация покажет пустоту');
});

test('половина обновления не оставляет приложение мёртвым', () => {
    // index.html и js/*.js — разные файлы, и на медленной связи браузер может взять
    // одни из сети, а другие из кеша. Тогда разметка новая, код старый, и приложение
    // застревает без единой кнопки.
    const m = HTML.match(/function guardHalfUpdate\(\)[\s\S]*?\n        \}\)\(\);/);
    assert(m, 'страховки от половинчатого обновления нет');
    assert(/typeof Progress === 'undefined'/.test(m[0]),
        'проверка идёт не через typeof — Progress объявлен как const и в window не попадает');
    assert(/sessionStorage/.test(m[0]) && /location\.reload\(\)/.test(m[0]),
        'нет одноразовой перезагрузки — либо не чинится, либо зациклится');
});

group('Выход из профиля');

test('выход убирает профиль с устройства, а не просто переключает', async () => {
    // Если профиль остаётся, он виден в списке и открывается обратно одним касанием
    // без пароля. Тогда «выйти» означало бы «переключиться», а это у нас уже есть.
    const disk = {};
    const a = loadProgress(disk);
    a.P.switchTo('MASHA', 'secret12');
    a.P.setToken('MASHA', 'tok');
    a.P.recordAnswer('integer+:add:1', 'correct', 1000);
    eq(a.P.logout(), true, 'выход не состоялся');
    eq(a.P.getCode(), null, 'после выхода профиль всё ещё активен');

    const b = loadProgress(disk);              // «закрыли и открыли заново»
    eq(b.P.getCode(), null, 'после перезапуска вернулись в тот же профиль');
    const codes = b.P.listProfiles().map(p => p.code || p);
    assert(codes.indexOf('MASHA') < 0,
        `профиль остался на устройстве после выхода: ${JSON.stringify(codes)}`);
});

test('выход уносит с собой ключи от аккаунта', async () => {
    // Токен живёт 90 дней и сам по себе является ключом. Оставить его на устройстве,
    // с которого вышли, — то же самое, что не выходить.
    const { P } = loadProgress();
    P.switchTo('MASHA', 'secret12');
    P.setToken('MASHA', 'tok');
    P.logout();
    eq(P.getTokenFor('MASHA'), null, 'токен остался на устройстве после выхода');
    eq(P.getPasswordFor('MASHA'), null, 'пароль остался на устройстве после выхода');
});

test('выходить не из чего — выход честно отказывает', async () => {
    const { P } = loadProgress();
    eq(P.logout(), false, 'выход «сработал» без активного профиля');
});

test('перед выходом прогресс отправляют на сервер и проверяют результат', () => {
    // Выход стирает локальную копию. Если отправить и не проверить, всё, что не успело
    // уехать, пропадёт молча — а это ровно то, чего мы весь день избегали.
    const m = HTML.match(/async function logoutHere\(\)[\s\S]*?\n        \}\n/);
    assert(m, 'logoutHere не найден — срез сломался');
    const flushAt = m[0].indexOf('Progress.flush(true)');
    const logoutAt = m[0].lastIndexOf('Progress.logout()');
    assert(flushAt >= 0, 'перед выходом прогресс на сервер не отправляется');
    assert(flushAt < logoutAt, 'профиль стирается раньше, чем прогресс уходит на сервер');
    assert(/if \(!sent\)/.test(m[0]),
        'результат отправки не проверяется — потеря пройдёт молча');
});

test('гостя предупреждают, что выход сотрёт всё', () => {
    const m = HTML.match(/async function logoutHere\(\)[\s\S]*?\n        \}\n/);
    assert(/isGuest\(\)/.test(m[0]), 'гость выходит по общему пути, а терять ему больше всех');
    assert(/danger: true/.test(m[0]), 'необратимый выход гостя не помечен опасным');
});

group('Свой логин и пароль');

test('логин можно скопировать, пароль — нельзя показать', () => {
    assert(/function copyOwnCode\(\)/.test(HTML), 'логин нельзя скопировать');
    // Пароля нет ни на устройстве (стирается после обмена на токен), ни на сервере
    // (только хэш). Любая кнопка «показать пароль» в профиле была бы обманом.
    assert(!/getPasswordFor\([^)]*\)[^;]*innerText|Показать пароль аккаунта|Твой пароль:/.test(HTML),
        'в профиле появился показ пароля — прочитать его неоткуда, это обман');
});

test('свой пароль можно сменить, и приложение берёт новый токен', () => {
    // Сервер гасит все сессии при смене, включая текущую. Без нового токена приложение
    // продолжит ходить со старым и получит отказ на первом же запросе.
    const m = HTML.match(/async function changeOwnPassword\(\)[\s\S]*?\n        \}\n/);
    assert(m, 'смены своего пароля в приложении нет');
    assert(/session_change_own_password/.test(m[0]), 'зовётся не та функция сервера');
    assert(/session_login/.test(m[0]) && /setToken/.test(m[0]),
        'после смены пароля новый токен не берётся — приложение останется с мёртвым');
});

test('ученику смену пароля не предлагают', () => {
    // На сервере это запрещено (not_allowed), и прятать кнопку правильнее, чем
    // показывать её и потом объяснять отказ.
    const m = HTML.match(/async function changeOwnPassword\(\)[\s\S]*?\n        \}\n/);
    assert(/getAccountType\(\) === 'linked'/.test(m[0]),
        'ученик может попытаться сменить пароль и получит непонятный отказ');
});
