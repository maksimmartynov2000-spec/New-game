...
test('из профиля возвращаемся в профиль, а не на экран входа', () => {
    const m = HTML.match(/function closeRegisterScreen\(\)[\s\S]*?\n        \}/);
    assert(m, 'closeRegisterScreen не найдена — срез сломался');
    assert(/registerCameFromProfile/.test(m[0]),
        'возврат из регистрации не помнит, откуда пришли — гостя выбросит на вход');
});
