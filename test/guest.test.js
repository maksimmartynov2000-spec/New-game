...
test('гость видит только положительные числа', () => {
    // Гость записан как 'self', а «self» исторически означало репетитора, которому
    // открыто всё. Из-за этого гостю показывали отрицательные, десятичные и дроби —
    // три раздела, которые мы нарочно прячем, пока до них не дошли.
    const m = HTML.match(/function hasFullAccess\(\)[\s\S]*?\n        \}/);
    assert(m, 'hasFullAccess не найдена — срез сломался');
    const first = m[0].split('\n').find(l => /return/.test(l) && !/^\s*\/\//.test(l));
    assert(/isGuest/.test(m[0]),
        'полный доступ не отличает гостя — ему снова видны все четыре раздела');
    // Проверка про гостя должна стоять ДО общего «не ученик — значит всё открыто»,
    // иначе она никогда не сработает.
    assert(m[0].indexOf('isGuest') < m[0].indexOf("!== 'linked'"),
        'проверка гостя стоит после общего правила и потому недостижима');
});
