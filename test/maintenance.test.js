...
test('в файле по умолчанию работы выключены', () => {
    // Иначе первый же мерж запер бы всех.
    assert(/until:\s*null/.test(FLAG_FILE), 'в content/maintenance.js флаг оставлен включённым');
});
...
test('время округляется вверх, а не вниз', () => {
    // «Через 3 минуты» — обещание, которое не выполнится: ученик вернётся ровно
    // через три и упрётся в ту же заглушку.
    const w = load({ until: inMinutes(3) });
    const text = w.M.maintenanceWaitText(w.M.maintenanceLeftMs());
    assert(/5/.test(text), `округлили вниз: ${text}`);
});

test('склонение минут не хромает', () => {
    const w = load({ until: inMinutes(1) });
    eq(w.M.maintenanceWaitText(5 * 60000), 'Приходи примерно через 5 минут');
    eq(w.M.maintenanceWaitText(21 * 60000), 'Приходи примерно через 25 минут');
    eq(w.M.maintenanceWaitText(60 * 60000), 'Приходи примерно через 60 минут');
});
