...
test('у скорости тот же порог выборки', () => {
    // Точность здесь задана нарочно: проверка про РАЗМЕР ВЫБОРКИ, а ступени
    // скорости с некоторых пор требуют ещё и бронзы по точности — см. ниже.
    const fast = { speedSec: 1.0, accuracy: 100 };
...
test('золото по точности достижимо с ошибками', () => {
    ...
    eq(L.ladderTierByMetrics('s', metrics({ count: 100, attempts: 110, accuracy: 91, speedSec: 1.0 }), 1, 'integer'), 5, 'скорость тоже не заперта');
});

group('Скорость без точности не считается');

// Замер: «ученик», который вообще не считает, а выбирает по виду четырёх чисел,
// набирает 34–44% верных. Пороги скорости он проходит все и за несколько минут —
// они меряются по ВЕРНЫМ ответам, а верные при угадывании набегают сами.
// Поэтому ступень скорости требует бронзы по точности.
test('без бронзы по точности скорость не даётся', () => {
    const tapping = metrics({ count: 250, attempts: 570, accuracy: 44, speedSec: 0.5 });
    eq(L.ladderTierByMetrics('s', tapping, 1, 'integer'), 0, 'сорок четыре процента — не скорость');
    eq(L.ladderTierByMetrics('a', tapping, 1, 'integer'), 0, 'точность тоже ниже бронзы');
});

test('с бронзой по точности скорость даётся полностью', () => {
    const honest = metrics({ count: 250, attempts: 300, accuracy: 80, speedSec: 0.5 });
    eq(L.ladderTierByMetrics('s', honest, 1, 'integer'), 5, 'ровно бронза уже пускает');
});

test('порог скорости — именно бронза, а не отдельное число', () => {
    // Если завтра бронзу по точности сдвинут, порог для скорости обязан
    // поехать вместе с ней: «бронза» во всех трёх лесенках значит одно и то же.
    const justBelow = metrics({ count: 250, accuracy: L.ACCURACY_TIERS[0] - 1, speedSec: 0.5 });
    const justAt = metrics({ count: 250, accuracy: L.ACCURACY_TIERS[0], speedSec: 0.5 });
    eq(L.ladderTierByMetrics('s', justBelow, 1, 'integer'), 0, 'на процент ниже бронзы');
    eq(L.ladderTierByMetrics('s', justAt, 1, 'integer'), 5, 'на самой бронзе — как позволяет время');
});

test('количество этим не закрывается — и не должно', () => {
    // Там считаются просто верные ответы за всё время, и отличить набитые
    // от заработанных нельзя ни по времени, ни по точности. Так и записано.
    const tapping = metrics({ count: 250, attempts: 570, accuracy: 44, speedSec: 0.5 });
    eq(L.ladderTierByMetrics('c', tapping, 1, 'integer'), 5, 'количество берётся как прежде');
});
...
test('верхние ступени точности опущены, нижние и ворота — нет', () => {
    // 95 и 99 были недостижимы: при девяноста процентах до короны за четыреста
    // дней доходил один ученик из десяти. Опустили только четвёртую и пятую —
    // третья это ворота между звёздами, и трогать её значило бы менять доступ
    // к материалу, а не награду.
    eq(L.ACCURACY_TIERS.join(','), '80,85,90,93,97');
});

test('взятая ступень и текущий показатель — разные вещи', () => {
    // На карточке клетки они стоят рядом: значок берётся из заработанного и
    // остаётся навсегда, число — это последняя сотня ответов. Пока они совпадают,
    // разницы не видно; у того, кто раз взял легенду и потом сдал, была бы корона
    // рядом с 80% без единого слова объяснения. Отсюда подпись «сейчас» и отметка
    // «ниже взятой ступени»: заработанное не отнимаем, но и не врём.
    const slipped = {
        byTopic: { 'integer+:sub:1': { correct: 120, wrong: 20 } },
        daily: { '2026-09-01': { t: { 'integer+:sub:1': [80, 20, 0, 640000, 80] } } }
    };
    const m = L.topicMetrics(slipped, 'integer+:sub:1');
    eq(m.accuracy, 80, 'точность сейчас');
    eq(L.ladderTierByMetrics('a', m, 1, 'integer'), 1, 'по нынешним числам это бронза');
    eq(L.ladderTierByMetrics('s', m, 1, 'integer'), 2, 'восемь секунд — серебро');

    // А пока показатель держится, расхождения нет и отметки быть не должно.
    const held = {
        byTopic: { 'integer+:sub:1': { correct: 120, wrong: 0 } },
        daily: { '2026-09-01': { t: { 'integer+:sub:1': [100, 0, 0, 340000, 100] } } }
    };
    const m2 = L.topicMetrics(held, 'integer+:sub:1');
    eq(m2.accuracy, 100, 'точность держится');
    eq(L.ladderTierByMetrics('a', m2, 1, 'integer'), 5, 'сто процентов — легенда');
    eq(L.ladderTierByMetrics('s', m2, 1, 'integer'), 4, '3,4 секунды — алмаз');
});
