...
test('если в условии названа единица, она есть и в ответе', () => {
    // «На отметке −13 метров… Где он теперь?» — «−27». Ребёнок читает голое
    // число и не знает, чего именно двадцать семь: метров, рублей или очков.
    // Правило пришло от репетитора после чтения живых задач: у каждого числа
    // должна быть своя единица, и в условии, и в ответе.
    //
    // Проверяем узко и потому надёжно: ловится только случай «в условии единица
    // есть, а ответ — голое число». Задачи про саму запись («Чему равно −3 × 90?»
    // → «−270») единиц не называют и сюда не попадают, и это правильно: там
    // единиц нет вовсе.
    const UNITS = /градус|метр|рубл|очк|этаж|лет[^а-яё]|год|минут|час[^а-яё]/i;
    const BARE = /^[−-]?\d+$/;
    const bad = [];
    CELLS.forEach(key => ['diamond', 'legend'].forEach(tier => {
        const x = RU[key][tier];
        if (UNITS.test(x.task) && BARE.test(x.answer.trim())) {
            bad.push(`${key} ${tier}: «${x.answer}»`);
        }
    }));
    assert(bad.length === 0, bad.join('; '));
});
