...
test('у каждого языка свой флаг из двух букв-регионов', () => {
    // Флаг в эмодзи — это ДВА символа-указателя региона, а не картинка. Одна буква,
    // обычный смайлик или пустая строка внешне сойдут за флаг в редакторе, а на
    // экране дадут квадрат или чужой значок. Проверяем именно устройство символа.
    const flags = [];
    D.langs.forEach(l => {
        const cp = [...(l.flag || '')].map(c => c.codePointAt(0));
        eq(cp.length, 2, `у ${l.code} флаг не из двух символов: «${l.flag}»`);
        cp.forEach(c => assert(c >= 0x1F1E6 && c <= 0x1F1FF,
            `у ${l.code} в флаге не буква региона: «${l.flag}»`));
        flags.push(l.flag);
    });
    eq(new Set(flags).size, flags.length, 'уникальных флагов');
});

test('в списке языков флаг стоит РЯДОМ с названием, а не вместо него', () => {
    // Язык не равен стране, и на Windows флаги не рисуются вовсе. Если название
    // из строки уберут, на таком экране останется «RU» буквами — и выбрать язык
    // будет не по чему.
    const from = D.script.indexOf('function openLangPicker()');
    const to = D.script.indexOf('function refreshLangButtons()', from);
    assert(from >= 0 && to > from, 'не найден выбор языка');
    const src = D.script.slice(from, to);
    assert(/lang-option-flag'\)\.innerText\s*=\s*l\.flag/.test(src),
        'в строке списка нет флага языка');
    assert(/lang-option-name'\)\.innerText\s*=\s*l\.name/.test(src),
        'из строки списка пропало название языка');
});
