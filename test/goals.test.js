...
test('строка называет медаль, клетку, порог и текущее', () => {
    const w = load({ 'integer+:add:1': { count: 20 } });
    // Медаль называется так же, как везде: значок и имя в кавычках. Раньше здесь
    // стояло голое «Бронза» — то же слово, но выглядело как ещё одно название темы.
    eq(w.G.nearestGoalText(w.st, {}, 'c'),
       'Ближайшая цель — 🥉 «Бронза» в теме integer+:add:1: 25, сейчас 20');
});
