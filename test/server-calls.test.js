...
// Единственные функции, которые приложение зовёт напрямую: вход (он же обмен пароля
// на токен), регистрация, чтение и запись состояния. Всё остальное идёт через
// callAuthed, который сам подставляет токен.
//
// Почему регистрация здесь, а не за callAuthed. callAuthed подтверждает личность
// токеном — а у того, кто регистрируется, личности ещё нет, и токен ему как раз
// предстоит получить. Это ровно тот же случай, что вход: оба вызова стоят ДО того,
// как человек становится кем-то, и оба поэтому несут пароль. Правило, которое эти
// проверки сторожат, от такого расширения не размывается: пароль по-прежнему уходит
// только туда, где личность устанавливается, и больше никуда.
const DIRECT = ['session_login', 'session_register', 'session_state', 'session_save'];
...
test('пароль не уходит на сервер ни в одном вызове, кроме входа', () => {
    // p_password есть у входа (session_login), у регистрации (session_register),
    // у удаления аккаунта (там он нужен как подтверждение) и у смены пароля. Ни один
    // ДРУГОЙ вызов пароля слать не должен — ради этого пароль и меняли на токен.
    //
    // Регистрация в исключениях по той же причине, что вход: она СОЗДАЁТ личность,
    // и подтверждать её токеном нечем — токен она как раз и выдаёт в ответ.
    const PASSWORD_ALLOWED = ['session_login', 'session_register'];
    const bad = [];
    const re = /\.rpc\(\s*'([a-z_]+)'[\s\S]{0,300}?\)/g;
    let m;
    while ((m = re.exec(ALL))) {
        if (PASSWORD_ALLOWED.includes(m[1])) continue;
