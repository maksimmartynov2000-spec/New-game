...
-- заводится сам себе хозяин: owner_code пустой, account_type 'self'. Ворота по звёздам
-- на таких не распространяются (см. levelGateApplies в index.html) — и это верно:
-- ребёнку с улицы некому открывать звёзды вручную.
--
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'accountType', 'self', 'ownerCode', null
    )
  );
